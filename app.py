...
def main():
    ...
    str_menu_option = st.sidebar.radio(
        ...
        ["Paso 1: Home", "Paso 2: Carga del dataset", "Paso 3: Análisis (EDA)", "Paso 4: Conclusiones"]
    )

    if str_menu_option == "Paso 1: Home":
        mostrar_home()
    elif str_menu_option == "Paso 2: Carga del dataset":
        cargar_datos()
    elif str_menu_option == "Paso 3: Análisis (EDA)":
        mostrar_eda()
    elif str_menu_option == "Paso 4: Conclusiones":
...
def mostrar_conclusiones():
    ...
    1. Si analizamos MonthlyChanges vs Churn podemos concluir que aquellos Clientes cuya facturación mas alta son los que se han ido en mayor proporción. Se les podria ofrecer a esos clientes cada cierto tiempo alguna oferta que les brinde un valor agregado.
    2. Si analizamos Contract vs Churn , revisando que la mayor cantidad de Clientes que se fueron tenian contratos *Month-to-Month* . Ofrecer contratos de minimo 1 año con mayores beneficios que los mensuales para que el cliente opte por ellos.
    3. Si analizamos tenure vs Churn podemos concluir que la mayor cantidad de Clientes que se fueron son los que tienen menos de 30 meses de permanencia. El mayor volumen de abandonos ocurre durante el periodo de "tenure" más bajo (clientes nuevos). Se requiere hacer seguimiento a los clientes y el servicio. Encuestas de satisfacción , mejora de algún servicio o un bono durante los primeros seis meses.
    3. Si analizamos los servicios *OnlineSecurity* o *TechSupport* aquellos Clientes que no los tienen tienen mayor probabilidad de irse. Programa "Demo" de 1,2,3,6 meses con ese servicio. A mayor cantidad de servicios mayor oferta. Ayuda un Marketing que les muestre a los clientes la importancia de estos servicios adicionales.
    4. Si analizamos PaymentMethod vs Churn podemos revisar que es muy alto la tasa de abandonos por Electronic Check. Promover Credit Card u optros medios de pago que me brinden mayor seguridad.
    5. Si analizamos InternetService vs Churn :Esta relacionado a los servicios con Fiber Optic.Se debe revisar a detalle para encontrar porque hay mayor cantidad de abandonos en comparacion a otros medios como DSL.
    """)
